...
Vi skal nå se på noe vi kaller `løkker', som kommer til å spare oss for masse jobb. En `løkke' er enkelt forklart noe som lar oss gjenta kode, litt på samme måte som en funksjon. Det finnes to typer løkker i Python, og vi kaller de \verb+for+- og \verb+while+-løkker. La oss starte med å se på \verb+for+-løkker.

\subsubsection*{For-løkker}

En for-løkke gjentar en bit med kode for hvert element i en liste. La oss for eksempel se på en liste med navn på elever i en klasse, vi kan da gjør for eksempel dette

\begin{lstlisting}
students = ["Mary", "James", "Siri", "Alesha"]
...
\end{verbatim}
Så vi ser at hvert navn i lista blir skrevet ut til terminalen, på hver sin linje. I hvert tilfelle blir det også skrevet ut hvor mange karakterer det er i navnet.

En for-løkke må alltid begynnes med nøkkelorder \verb+for+, derav navnet. Det som står rett etter \verb+for+-nøkkelordet skal være navnet på en variabel som skal opprettes, du bestemmer dette navnet, og det kan være hva som helst. Så kommer nøkkelordet \verb+in+, så må det være en liste og til slutt kolon, \verb+:+. Altså kan en \verb+for+-løkke startes som dette
\begin{lstlisting}
for my_chosen_name in some_list:
\end{lstlisting}
Koden som følger får et innrykk, akkurat som for tester og funksjoner. All kode som har et innrykk hører til løkken, og kommer til å bli gjentatt for hvert element i lista. For hvert element, så leser Python ut innholdet av det elementet og legger inn i variabelen vi her har kalt \verb+my_chosen_name+. Altså, elementet i lista brukes ved at du gir den navnet du har spesifisert i definisjonen av for-løkka. La oss tegne en kjapp skisse

\begin{centering}
	\includegraphics[width=\textwidth]{for_loop}
\end{centering}

La oss se på et par eksempler vi kan kjøre
\begin{lstlisting}
for i in [1, 2, 3, 4]:
	print i**2
\end{lstlisting}
Her sier jeg at vi skal løkke over listen \verb+[1, 2, 3, 4]+, og for hvert element som behandles skal vi kalle det \verb+i+. Det vi gjør med vært element er å skrive ut kvadratet av tallet. Resultatet blir som følger
\begin{verbatim}
1
4
9
...
\begin{itemize}
	\item[(a)] Definer en liste av de forskjellige lotto-tallene.
	\item[(b)] Bruk \verb+.pop()+ til å trekke ut 7 hovedtall og 3 bonustall.
...
\subsection*{Oppgave 7 --- Fibonacci-tall (OBS! Utfordrende)}
...
Skriv en funksjon som tar $n$ som argument, og returnerer det $n$-te Fibonnaci-tallet. \textbf{(Spør gjerne om hjelp her!)}
